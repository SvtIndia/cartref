            ...
            <div class="flex flex-wrap items-center gap-2">
              <div class="relative">
                <select title="Status" v-model="status" @change="fetchProduct()"
                        class="block appearance-none pl-2 pr-8 w-auto leading-tight h-full cursor-pointer text-black bg-white border border-gray-400 focus:outline-none hover:shadow-sm focus:ring-2 focus:ring-green-500 focus:border-none font-medium rounded-lg text-sm px-3 py-2">
              ...
              <div class="relative">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 cursor-pointer" @click="keyword = ''; fetchProduct();" v-if="keyword">
              ...
              <div class="flex border border-gray-600 rounded-lg bg-white">
                <button class="px-2 py-1 m-[2px] hover:bg-green-100 border-r border-solid cursor-pointer" @click="fetchProduct()">
                  <i class="ffi fi-rr-refresh mr-1"></i>
                </button>
                <select class="w-14 block px-1 m-[2px] text-base text-center text-gray-900 bg-white cursor-pointer" @change="fetchProduct()" v-model="row_count">
          ...
          <template v-else-if="products && products.length > 0">
            <div class="clear-right overflow-x-auto">
              <div class="table border-solid border border-gray-500 w-full">
                ...
                <div v-for="(product, index) in products" v-bind:key="index" class="table-row table-body hover:bg-green-100">
                  ...
                  <div class="table-cell border-t border-l border-gray-500 text-sm p-1 text-center !align-middle">
                    <img @click="imageModal($store.state.storageUrl + product.image)" v-if="product.image"
                         class="w-20 h-20 border border-gray-400 mx-auto p-1 rounded-[50%]" :src="$store.state.storageUrl + product.image"
                         :alt="product.name">
                      ...
                      <div class="flex gap-8 w-full">
                        <div class="">
                          ...
                          <div class="flex items-center gap-2">
                            ...
                            <div class="font-normal text-gray-500">{{ product.brand_id ?? '-' }}</div>
                          </div>
                        </div>
                        <div>
                          <div class="flex items-center gap-2">
                            <div class="font-medium">SKU:</div>
                            <div class="font-normal text-gray-500">{{ product.sku ?? '-' }}</div>
                          </div>
                          <div class="flex items-center gap-2">
                  ...
                  <div class="table-cell border-t border-l border-gray-500 text-sm px-1 text-center py-1 !align-middle">
                    <div class="font-normal text-gray-900" v-html="formDateTime(product.updated_at)"></div>
                  </div>
                  <div class="table-cell border-t border-l border-gray-500 text-sm align-[middle!important] text-center">
                    <div class="flex gap-4 items-center justify-center">
                      <a href="javascript:void(0)" type="button" class="font-medium cursor-pointer text-yellow-500">
