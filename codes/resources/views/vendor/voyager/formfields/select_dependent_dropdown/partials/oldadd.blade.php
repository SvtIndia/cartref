	...
	@if (!count($errors))	
		...
		<div class="{{ $css }}" style="padding: 10px;">
			<label for="{{$row->details->name}}" class="required">{{$row->details->display}}</label>
		...
		@foreach ($selects as $key => $item)
			...
			@if (isset($selects[$next]))
				<label for="{{$item->name}}" class="required">{{$item->display}}</label>
				<select id="{{ $item->name }}" name="{{ $item->name }}" class="form-group select2 dependent-dropdown"
						data-route="{{ route($row->details->route) }}"
						
						@if (!empty($selects[$next]->wherehas) OR empty($selects[$next]->where))
							data-params="{{ json_encode(['options' => $selects[$next], 'model' => $selects[$next]->model, 'value' => '__value']) }}"
						@else
			...
			@else
				...
				<select id="{{$item->name}}" name="{{$item->name}}" class="form-group select2" placeholder="{{$item->placeholder}}" @if(!empty($item->wherehas))  data-placeholder="Select {{ $item->display }}s" multiple @endif>
					@if(isset($item->placeholder))
					{{-- <option value="0" selected="selected">{{ $item->placeholder }}</option> --}}
					@endif
				</select>
