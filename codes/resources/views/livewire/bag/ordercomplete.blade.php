            ...
            <div class="container mt-8">
                ...
                <div class="order-actions">
                    <div class="float-left">
                        <a href="{{ route('myorders') }}" class="btn btn-icon-left btn-dark btn-rounded btn-md mb-4"><i class="d-icon-arrow-left"></i> My Orders</a>
                    </div>
                    <div class="float-right">
                        ...
                        @if ($this->canbecancelled == true)
                            <a wire:click="cancelorder" wire:loading.remove class="btn btn-icon-right btn-light btn-rounded btn-md mb-4 ml-3">Cancel Order<i class="fa fa-times-circle"></i></a>
                        ...
                        @else
                            <a class="btn btn-icon-right btn-light btn-rounded btn-md mb-4 ml-3 btn-disabled" title="Order cannot be cancelled!">Cancel Order<i class="fa fa-times-circle"></i></a>
                        @endif
                        ...
                        <form action="{{ route('downloadinvoice', ['id' => $this->orderid]) }}" method="post">
                            ...
                            <button type="submit" class="btn btn-icon-right btn-light btn-rounded btn-md mb-4">Download Invoice <i class="fa fa-cloud-download-alt"></i></button>
                        </form>
                    ...
                    <table class="order-details-table table-responsive">
                        ...
                        <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="product-name name">
                                            ...
                                            @if (!empty($item->cost_per_g))
                                                <br><span>Cost per G+: {{ Config::get('icrm.currency.icon') }}{{ $item->cost_per_g }}/-</span>    
                                            @endif
                                            
                                            @if (Config::get('icrm.site_package.multi_vendor_store') == 1)
                                                <br><span>Vendor: <a href="{{ route('products.vendor', ['slug' => $item->vendor->id]) }}">{{ ucwords($item->vendor->brand_name) }}</a></span>
                                            @endif

                                            <br><span>Order type: {{ $item->type }}</span>

                                            @if (!empty($item->requirement_document))
                                                <br><span>Custom Requirement: <a href="{{ asset($item->requirement_document) }}" style="color: blue">Download</a></span>
                                            @endif

                                            @if (!empty($item->customized_image))
                                                <br><span>Customized image: <a href="{{ asset($item->customized_image) }}" target="_blank" style="color: blue">Download</a></span>
                                            @endif
                                            
                                            @if (!empty($item->original_file))
                                                <br><span>Original file: 
                                                    @foreach (collect(json_decode($item->original_file)) as $key => $file)
                                                        <a href="{{ $file }}" target="_blank" style="color: blue">Attachment {{ $key+1 }} @if($loop->last) @else , @endif</a>
                                                    @endforeach
                                                </span>    
                                            @endif
                                        ...
                                        <td class="product-amount">
                                            ...
                                            @if ($item->order_status == 'New Order' OR $item->order_status == 'Scheduled For Pickup' OR $item->order_status == 'Ready To Dispatch' OR $item->order_status == 'Shipped')
                                                <br><span>Expected delivery by {{ date('d-M-Y', strtotime($items->first()->exp_delivery_date)) }}</span>  
                                            @endif
                                        ...
                                        <td class="product-amount">
                                            ...
                                            @if (Config::get('icrm.shipping_provider.shiprocket') == 1)
                                                @if ($item->type == 'Regular')
                                                    @if (!empty($item->tracking_url))
                                                        <a href="{{ $item->tracking_url }}" target="_blank" class="btn btn-sm btn-light">Track</a>
                                                    @else
                                                        <a title="Shipping not initiated yet" target="_blank" class="btn btn-sm btn-light btn-disabled">Track</a>
                                                    @endif
                                                @endif
                                            @endif
                                            ...
                                            @if (Config::get('icrm.order_lifecycle.return.feature') == 1)
                                                ...
                                                @else
                                                    @if ($item->type == 'Regular')
                                                        <a title="Will be enabled once the order is delivered" class="btn btn-sm btn-light btn-disabled">Returns</a>    
                                                    @endif
                            ...
                            <tr class="summary-subtotal mt-10">
                                ...
                                <td class="summary-subtotal-price" style="text-align: left;">{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_value, 2) }}</td>
                            </tr>
                            ...
                            <tr class="summary-subtotal">
                                ...
                                <td class="summary-subtotal-price" style="text-align: left;">{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_discount, 2) }}</td>
                            </tr>
                            @endif
                            ...
                            @if ($item->type == 'Showcase At Home')
                                <tr class="summary-subtotal">
                                    ...
                                    <td class="summary-subtotal-price" style="text-align: left; color: red;">-{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_deliverycharges, 2) }}</td>
                                </tr>
                            @else
                                <tr class="summary-subtotal">
                                    <td></td>
                                    ...
                                    @if ($item->order_deliverycharges > 0)
                                        <td class="summary-subtotal-price" style="text-align: left; color: green;">+{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_deliverycharges, 2) }}</td>    
                                    @else
                                        <td class="summary-subtotal-price" style="text-align: left; color: green;">Free Shipping</td>
                                    @endif
                            ...
                            <tr class="summary-subtotal">
                                ...
                                <td class="summary-subtotal-price" style="text-align: left;">{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_subtotal, 2) }}</td>
                            </tr>
                            ...
                            @if ($item->order_tax > 0)
                                <tr class="summary-subtotal">
                                    ...
                                    <td class="summary-subtotal-price" style="text-align: left; color: green;">+{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_tax, 2) }}</td>
                                </tr> 
                            @endif
                            ...
                            <tr class="summary-subtotal">
                                ...
                                <td>
                                    <p class="summary-total-price" style="text-align: left;">{{ Config::get('icrm.currency.icon') }}{{ number_format($item->order_total, 2) }}</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="address-info pb-8 mb-6">
                    ...
                    <p class="address-detail pb-2">
                        ...
                        {{ $items->first()->dropoff_pincode }}, {{ $items->first()->dropoff_city }}, {{ $items->first()->dropoff_state }}<br>
