            ...
            @if (!empty($value['attributes']['color']))
                ...
                @if (!empty($colorimage))
                    @if (!empty($colorimage->main_image))
                        <img style="width: 4rem; height:4rem;" src="{{ Voyager::image($colorimage->main_image) }}"
                            alt="{{ $value['product']['name'] }}">
                    @else
                        <img style="width: 4rem; height:4rem;" src="{{ Voyager::image($value['product']['image']) }}"
                            alt="{{ $value['product']['name'] }}">
                    @endif
                @else
                    <img style="width: 4rem; height:4rem;" src="{{ Voyager::image($value['product']['image']) }}"
            ...
            <div class="info" style="color:black;">
                ...
                <div>
                    ...
                    @if (!empty($value['attributes']['requirement_document']))
                        <div>
                            Requirements: <a href="{{ $value['attributes']['requirement_document'] }}"
                    ...
                    @if (!empty($value['attributes']['customized_image']))
                        <div>
                            Customized Image:
                            <a href="{{ $value['attributes']['customized_image'] }}" style="color: blue;">Download</a>
                        </div>
                    @endif
                    ...
                    @if (!empty($value['attributes']['original_file']))
                        <div>
                            ...
                            @foreach ($originalfiles as $key => $originalfile)
                                <a href="{{ $originalfile }}" target="_blank" style="color: blue;">Attachment
                                    {{ $key + 1 }} @if ($loop->last)
                                    @else,
                                    @endif
                                </a>
