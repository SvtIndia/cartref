        ...
        @if (Config::get('icrm.frontend.menu.dynamic_category') == 1)
                    ...
                    @if (count($categories) > 0)
                            ...
                            @if (count($genders) > 0)
                                ...
                                @php
                                    $genderwisecategories = App\ProductCategory::where('status', 1)
                                            ...
                                            })->with(['subcategory'])->orderBy('order_id', 'ASC')->get();
                                @endphp
                                 ...
                                 <li class="submenu @if($menu_item->link() == Route::getFacadeRoot()->current()->uri()) active @endif
                                    ...
                                    @foreach ($genderwisecategories as $category)
                                        <li>
                                            <a href="{{ route('products.category', ['category' => $category->slug, 'gender['.$gender->name.']' => $gender->name]) }}">{{ $category->name }}</a>
                                            @if (count($category->subcategory) > 0)
                                                ...
                                                <ul>
                                                    @foreach ($subcatgories as $subcategory)
                                                        <li>
                                                            <a href="{{ route('products.subcategory', ['subcategory' => $subcategory->slug, 'gender['.$gender->name.']' => $gender->name]) }}">{{ $subcategory->name }}</a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                                @endif
                        ...
                        @elseif(Config::get('icrm.frontend.header.categoryonrow') == 1)
                            ...
                            @foreach ($categories as $category)
                                ...
                                <li class="submenu @if($menu_item->link() == Route::getFacadeRoot()->current()->uri()) active @endif">
                                    <a href="{{ route('products.category', ['category' => $category->slug]) }}">{{ $category->name }}</a>
                                    @if (count($category->subcategory) > 0)
                                        <ul>
            ...
            @else
                ...
                <li class="
                    ...
                    @if($menu_item->link() == Route::getFacadeRoot()->current()->uri()) active @endif
                    ">
