...
@section('meta-seo')
    <title>{{ $slider->title_1 }}</title>
    <meta name="keywords" content="{{ $slider->title_1 }}, {{ $slider->title_2 }}">
    <meta name="description" content="{{ $slider->small_description }}">
@endsection
    ...
    <nav class="breadcrumb-nav">
        <div class="container">
            <ul class="breadcrumb">
                ...
                <li>{{ Str::limit($slider->title_1, 50) }}</li>
            </ul>
        </div>
    </nav>
        ...
        <div class="container">
            <div class="row gutter-lg">
                <div class="col-lg-12">
                    ...
                    <nav class="page-nav">
                        @isset($previous)
                            <a class="pager-link pager-link-prev" href="{{ route('slider.slug', ['slug' => $previous->id, 'title' => $previous->title_1]) }}">
                                Previous Slider
                                <span class="pager-link-title">{{ $previous->title_1 }}</span>
                            </a>
                        @else
                        ...
                        @isset($next)
                            <a class="pager-link pager-link-next" href="{{ route('slider.slug', ['slug' => $next->id, 'title' => $next->title_1]) }}">
                                Go To Slider
                                <span class="pager-link-title">{{ $next->title_1 }}</span>
                            </a>
                        @else
